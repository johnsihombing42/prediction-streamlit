    ...
    try:
        if not df.empty:
            ...
            if interval == "Harian":
                ...
                # input_value = last_price_date.strftime("%Y-%m-%d")

                # Format harga dengan dua digit dibelakang koma
                formatted_price1 = "{:.2f}".format(last_predicted_price)

                st.write(f"Prediksi harga pada {input_value} adalah : {formatted_price1} USD")

                if 1000 <= last_predicted_price <= 1500 or 15000 <=last_predicted_price<= 25000:
                    st.write("Harga tersebut masuk ke dalam zona akumulasi, Sangat cocok untuk membeli cryptocurrency!.")
                elif 1501 <= last_predicted_price <= 2500 or 25000<=last_predicted_price <=45000:
                    st.write("Harga cryptocurrency berada dalam kisaran yang stabil.")
                else:
                    st.write(" Harga tersebut masuk ke dalam zona distribusi, Sangat cocok untuk melakukan take profit")
                ...
                # Format tanggal tanpa jam
                formatted_date = last_price_date.strftime("%Y-%m-%d")

                # Format harga dengan dua digit dibelakang koma
                formatted_price = "{:.2f}".format(last_predicted_price)

                st.write(f"Prediksi harga pada {formatted_date} adalah : {formatted_price} USD")

                if 1000 <= last_predicted_price <= 1500 or 15000 <=last_predicted_price<= 25000:
                    st.write("Harga tersebut masuk ke dalam zona akumulasi, Sangat cocok untuk membeli cryptocurrency!.")
                elif 1501 <= last_predicted_price <= 2000 or 25000<=last_predicted_price <=45000:
                    st.write("Harga cryptocurrency berada dalam kisaran yang stabil.")
                else:
                    st.write(" Harga tersebut masuk ke dalam zona distribusi, Sangat cocok untuk melakukan take profit")
